...
t<-ggplot(pcoa.b.plot,aes(x=PCo1,y=PCo2,color=Nets)) + geom_point() +
  labs(title="PCoA to differentiate the different nets using biovolumes",
...
u<-ggplot(pcoa.std.plot,aes(x=PCo1,y=PCo2,color=Nets)) + geom_point(show.legend=F) +   
  labs(title="PCoA to differentiate the different nets using standardized biovolumes", 
       x = paste("PCo1 ",pPCo1," %"), y=paste("PCo2 ",pPCo2," %")) 
u
...
#nbss plot test
g<-BSS.plot(info.object,info.sample,info.sample[6,1])
g
